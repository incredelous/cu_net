...
with open(save_dir + 'mean_std_dict.pickle', 'wb') as f:
    pickle.dump(data_types_mean_std_dict, f, protocol=0)
...
for i in survival_id_dev_HGG:
    ...
    # combined_sequence_array = np.stack(np.asarray(all_3d_data), axis=0)
    # combined_sequence_array = np.transpose(combined_sequence_array, (3,2,1,0))
    # combined_sequence_array.astype(np.float32)
    # X_dev_seq_input.append(combined_sequence_array)
    # seg_img.astype(int)
    # X_dev_seq_target.append(seg_img)
    del all_3d_data
    gc.collect()
...
for i in survival_id_tr_HGG:
    ...
    # combined_sequence_array = np.transpose(combined_sequence_array, (3,2,1,0))
    # combined_sequence_array.astype(np.float32)
    # X_train_seq_input.append(combined_sequence_array)
    # seg_img.astype(int)
    # X_train_seq_target.append(seg_img)
    del all_3d_data
    print("finished {}".format(i))
...
print("X_train_input shape:{};\t X_train_target shape:{};\t X_dev_input shape:{};\t X_dev_target shape:{};\t input array shape:{};\t label array shape:{};"\
.format(len(X_train_input), len(X_train_target), len(X_dev_input), len(X_dev_target), X_train_input[0].shape, X_train_target[0].shape))
