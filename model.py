...
def cu_net(x, is_train=False, reuse=False, n_out=1):
    _, ns, nx, ny, nz = x.get_shape().as_list()#(batch_size, steps, width, height, channel)
    cmc_output_1, cmc_output_2, cmc_output_3, cmc_output_4 = [], [], [], []
    with tf.variable_scope('cu_net', reuse=reuse):
        for i in range(ns):
        ...
        clstm = ConvLSTMLayer(stack,
                              ...
                              filter_size=(3, 3),
                              feature_map=512,
                              n_steps=ns)# shape= (batch_size, 155, 15, 15, 512)
        unstack = UnStackLayer(clstm, axis=1, name='lstm_unstack')
        de_outputs = []
        for index, layer in enumerate(unstack):
            with tf.variable_scope('cu_net_decoder', reuse= True if index > 0 else None):
                ...
                up3 = ConcatLayer([up3, cmc_output_2[index]], 3, name='concat3')# shape= (batch_size, 60, 60, 256)
                conv3 = Conv2d(up3, 128, (3, 3), act=tf.nn.relu, name='uconv3_1')# shape= (batch_size, 60, 60, 128)
                conv3 = Conv2d(conv3, 128, (3, 3), act=tf.nn.relu, name='uconv3_2')
                up2 = DeConv2d(conv3, 64, (3, 3), (nx/2, ny/2), (2, 2), name='deconv2')# shape= (batch_size, 120, 120, 64)
